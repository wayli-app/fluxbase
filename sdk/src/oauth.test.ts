...
describe("OAuthProviderManager", () => {
      ...
      const mockProviders: OAuthProvider[] = [
        {
          ...
          redirect_urls: ["https://app.com/callback"],
          redirect_urls: ["https://app.com/callback"],
  ...
  describe("createProvider", () => {
    ...
    it("should create a provider with multiple redirect URLs", async () => {
      const mockResponse: CreateOAuthProviderResponse = {
        success: true,
        id: "provider-multi",
        provider: "github",
      ...
      const redirectUrls = [
        "https://app.com/api/v1/auth/oauth/github/callback",
      ...
      expect(mockFetch.post).toHaveBeenCalledWith(
        "/api/v1/admin/oauth/providers",
        expect.objectContaining({ redirect_urls: redirectUrls }),
      );
    });
  });
  ...
  describe("updateProvider", () => {
    ...
    it("should update the redirect URL list", async () => {
      const mockResponse: UpdateOAuthProviderResponse = {
        success: true,
        message: "OAuth provider 'GitHub' updated successfully",
      };
      ...
      const redirectUrls = [
        "https://new-domain.com/api/v1/auth/oauth/github/callback",
        "https://alt.new-domain.com/api/v1/auth/oauth/github/callback",
      ];
    ...
    it("should handle update errors", async () => {
      ...
      await expect(
        manager.updateProvider("nonexistent", { enabled: false }),
...
describe("AuthSettingsManager", () => {
  ...
  describe("update", () => {
    ...
    it("should handle update errors", async () => {
      ...
      await expect(manager.update({ password_min_length: 16 })).rejects.toThrow(
        "Update failed",
      );
    });
  });
