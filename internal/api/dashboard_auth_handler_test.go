...
func TestDashboardAuthHandler_Signup_Validation(t *testing.T) {
	app := fiber.New()
	handler := NewDashboardAuthHandler(nil, nil, nil, nil, nil, "", "", nil)

	app.Post("/dashboard/auth/signup", handler.Signup)
	...
	tests := []struct {
		...
		{
			name:           "missing email",
			body:           `{"password":"test123","full_name":"John"}`,
			expectedStatus: 400,
			expectedError:  "Email, password, and full name are required",
		},
		{
			name:           "missing password",
			body:           `{"email":"test@example.com","full_name":"John"}`,
			expectedStatus: 400,
			expectedError:  "Email, password, and full name are required",
		},
		{
		...
		{
			name:           "empty email",
			body:           `{"email":"","password":"test123","full_name":"John"}`,
			expectedStatus: 400,
			expectedError:  "Email, password, and full name are required",
		},
		{
			name:           "empty password",
			body:           `{"email":"test@example.com","password":"","full_name":"John"}`,
			expectedStatus: 400,
			expectedError:  "Email, password, and full name are required",
		},
		{
			name:           "empty full_name",
			body:           `{"email":"test@example.com","password":"test123","full_name":""}`,
			expectedStatus: 400,
			expectedError:  "Email, password, and full name are required",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest("POST", "/dashboard/auth/signup", strings.NewReader(tt.body))
...
func TestDashboardAuthHandler_Login_Validation(t *testing.T) {
	app := fiber.New()
	handler := NewDashboardAuthHandler(nil, nil, nil, nil, nil, "", "", nil)

	app.Post("/dashboard/auth/login", handler.Login)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest("POST", "/dashboard/auth/login", strings.NewReader(tt.body))
			req.Header.Set("Content-Type", "application/json")
...
func TestDashboardAuthHandler_RefreshToken_Validation(t *testing.T) {
	app := fiber.New()
	handler := NewDashboardAuthHandler(nil, nil, nil, nil, nil, "", "", nil)

	app.Post("/dashboard/auth/refresh", handler.RefreshToken)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest("POST", "/dashboard/auth/refresh", strings.NewReader(tt.body))
			req.Header.Set("Content-Type", "application/json")
...
func TestDashboardAuthHandler_VerifyTOTP_Validation(t *testing.T) {
	app := fiber.New()
	handler := NewDashboardAuthHandler(nil, nil, nil, nil, nil, "", "", nil)

	app.Post("/dashboard/auth/2fa/verify", handler.VerifyTOTP)
	...
	tests := []struct {
		...
		},
		{
			...
			body:           `{"code":"123456"}`,
			expectedStatus: 400,
			expectedError:  "User ID and code are required",
		},
		{
			name:           "missing code",
		...
		},
		{
			...
			body:           `{"user_id":"not-a-uuid","code":"123456"}`,
			expectedStatus: 400,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest("POST", "/dashboard/auth/2fa/verify", strings.NewReader(tt.body))
			req.Header.Set("Content-Type", "application/json")
...
func TestDashboardAuthHandler_ChangePassword_Validation(t *testing.T) {
	app := fiber.New()
	handler := NewDashboardAuthHandler(nil, nil, nil, nil, nil, "", "", nil)

	// Simulate RequireDashboardAuth setting user_id
	app.Post("/dashboard/auth/password/change", func(c *fiber.Ctx) error {
		c.Locals("user_id", testUUID)
	...
	tests := []struct {
		...
		},
		{
			...
			expectedStatus: 400,
			expectedError:  "Current password and new password are required",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest("POST", "/dashboard/auth/password/change", strings.NewReader(tt.body))
			req.Header.Set("Content-Type", "application/json")
...
func TestDashboardAuthHandler_DeleteAccount_Validation(t *testing.T) {
	app := fiber.New()
	handler := NewDashboardAuthHandler(nil, nil, nil, nil, nil, "", "", nil)

	app.Delete("/dashboard/auth/account", func(c *fiber.Ctx) error {
		c.Locals("user_id", testUUID)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest("DELETE", "/dashboard/auth/account", strings.NewReader(tt.body))
			req.Header.Set("Content-Type", "application/json")
...
func TestDashboardAuthHandler_UpdateProfile_Validation(t *testing.T) {
	app := fiber.New()
	handler := NewDashboardAuthHandler(nil, nil, nil, nil, nil, "", "", nil)

	app.Put("/dashboard/auth/profile", func(c *fiber.Ctx) error {
		c.Locals("user_id", testUUID)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest("PUT", "/dashboard/auth/profile", strings.NewReader(tt.body))
			req.Header.Set("Content-Type", "application/json")
...
func TestDashboardAuthHandler_EnableTOTP_Validation(t *testing.T) {
	app := fiber.New()
	handler := NewDashboardAuthHandler(nil, nil, nil, nil, nil, "", "", nil)

	app.Post("/dashboard/auth/2fa/enable", func(c *fiber.Ctx) error {
		c.Locals("user_id", testUUID)
		return handler.EnableTOTP(c)
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest("POST", "/dashboard/auth/2fa/enable", strings.NewReader(tt.body))
			req.Header.Set("Content-Type", "application/json")
...
func TestDashboardAuthHandler_DisableTOTP_Validation(t *testing.T) {
	app := fiber.New()
	handler := NewDashboardAuthHandler(nil, nil, nil, nil, nil, "", "", nil)

	app.Post("/dashboard/auth/2fa/disable", func(c *fiber.Ctx) error {
		c.Locals("user_id", testUUID)
		return handler.DisableTOTP(c)
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest("POST", "/dashboard/auth/2fa/disable", strings.NewReader(tt.body))
			req.Header.Set("Content-Type", "application/json")
...
func TestDashboardAuthHandler_RequestPasswordReset_Validation(t *testing.T) {
	app := fiber.New()
	handler := NewDashboardAuthHandler(nil, nil, nil, nil, nil, "", "", nil)

	app.Post("/dashboard/auth/password/reset", handler.RequestPasswordReset)

	t.Run("email service not configured", func(t *testing.T) {
		req := httptest.NewRequest("POST", "/dashboard/auth/password/reset", strings.NewReader(`{"email":"test@example.com"}`))
		req.Header.Set("Content-Type", "application/json")
		...
		body, _ := io.ReadAll(resp.Body)
		assert.Contains(t, string(body), "Email service is not configured")
		assert.Contains(t, string(body), "SMTP_NOT_CONFIGURED")
	})
}

func TestDashboardAuthHandler_VerifyPasswordResetToken_Validation(t *testing.T) {
	app := fiber.New()
	handler := NewDashboardAuthHandler(nil, nil, nil, nil, nil, "", "", nil)

	app.Post("/dashboard/auth/password/reset/verify", handler.VerifyPasswordResetToken)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest("POST", "/dashboard/auth/password/reset/verify", strings.NewReader(tt.body))
			req.Header.Set("Content-Type", "application/json")
...
func TestDashboardAuthHandler_ConfirmPasswordReset_Validation(t *testing.T) {
	app := fiber.New()
	handler := NewDashboardAuthHandler(nil, nil, nil, nil, nil, "", "", nil)

	app.Post("/dashboard/auth/password/reset/confirm", handler.ConfirmPasswordReset)
	...
	tests := []struct {
		...
		{
			name:           "empty new_password",
			body:           `{"token":"reset-token","new_password":""}`,
			expectedStatus: 400,
			expectedError:  "Token and new password are required",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest("POST", "/dashboard/auth/password/reset/confirm", strings.NewReader(tt.body))
			req.Header.Set("Content-Type", "application/json")
...
func TestDashboardAuthHandler_SAMLACSCallback_SAMLNotConfigured(t *testing.T) {
	...
	assert.Contains(t, location, "SAML%20not%20configured")
}
