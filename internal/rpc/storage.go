...
// AppendExecutionLog appends a log line to an execution
func (s *Storage) AppendExecutionLog(ctx context.Context, executionID string, lineNumber int, level, message string) error {
	query := `
		INSERT INTO rpc.execution_logs (execution_id, line_number, level, message, created_at)
		VALUES ($1, $2, $3, $4, $5)
	`

	_, err := s.db.Exec(ctx, query, executionID, lineNumber, level, message, time.Now())
	if err != nil {
...
func (s *Storage) GetExecutionLogs(ctx context.Context, executionID string) ([]*ExecutionLog, error) {
	query := `
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&logEntry.ID, &logEntry.ExecutionID, &logEntry.LineNumber,
			&logEntry.Level, &logEntry.Message, &logEntry.CreatedAt,
		)
		if err != nil {
...
// GetExecutionLogsSince retrieves logs for an execution after a specific line number
func (s *Storage) GetExecutionLogsSince(ctx context.Context, executionID string, afterLine int) ([]*ExecutionLog, error) {
	query := `
		SELECT id, execution_id, line_number, level, message, created_at
		FROM rpc.execution_logs
	...
	rows, err := s.db.Pool().Query(ctx, query, executionID, afterLine)
	if err != nil {
		return nil, fmt.Errorf("failed to get execution logs: %w", err)
	}
	defer rows.Close()
