...
type BranchingConfig struct {
	...
	AdminDatabaseURL     string        `mapstructure:"admin_database_url"`      // Database URL with CREATE DATABASE privileges (defaults to main database URL)
}
...
func (bc *BranchingConfig) Validate() error {
	...
	if bc.MaxBranchesPerUser > 0 && bc.MaxTotalBranches > 0 && bc.MaxBranchesPerUser > bc.MaxTotalBranches {
		return fmt.Errorf("branching max_branches_per_user (%d) cannot exceed max_total_branches (%d)",
			bc.MaxBranchesPerUser, bc.MaxTotalBranches)
	}
